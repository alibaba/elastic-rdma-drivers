...
#else
	void (*sk_data_ready)(struct sock *sk, int unsed);
#endif
	void (*sk_error_report)(struct sock *sk);
};
...
{
#ifdef HAVE_SOCK_OPS_GET_NAME_THREE_PARAM
	return s->ops->getname(s, (struct sockaddr *)a, 1);
#else
	int unused;

	return s->ops->getname(s, (struct sockaddr *)a, &unused, 1);
#endif
}
...
static inline int getname_local(struct socket *s, struct sockaddr_storage *a)
{
	...
	return s->ops->getname(s, (struct sockaddr *)a, &unused, 0);
#endif
}
