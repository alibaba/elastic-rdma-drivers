...
static void erdma_add_dev_to_list(struct erdma_dev *dev)
{
	...
	init_completion(&dev->unreg_completion);

	list_add_tail_rcu(&dev->dev_list, &dev_list);
}

static void erdma_device_put(struct erdma_dev *dev)
{
	ibdev_dbg(&dev->ibdev, "custom erdma_device_put called.\n");
	if (refcount_dec_and_test(&dev->refcount))
		complete(&dev->unreg_completion);
}
...
static void erdma_remove_dev_from_list(struct erdma_dev *dev)
{
	...
	/*
	* We cannot move forward after a list_del_rcu until the
	* grace period
	*/
	synchronize_rcu();
...
#if !defined(HAVE_IB_DEVICE_GET_BY_NAME) || \
	!defined(HAVE_IB_DEVICE_GET_BY_NETDEV)
static bool erdma_device_try_get(struct erdma_dev *dev)
{
...
{
	erdma_device_put(to_edev(device));
}
#endif
					...
					unsigned int driver_id)
{
	...
	pr_debug("custom ib_device_get_by_name called.\n");
					  ...
					  unsigned int driver_id)
{
	...
	pr_debug("custom ib_device_get_by_netdev called.\n");

	rcu_read_lock();

	list_for_each_entry_rcu(edev, &dev_list, dev_list) {
		if (ndev == edev->netdev && erdma_device_try_get(edev)) {
			ibdev = &edev->ibdev;
			break;
		}
	}
...
int ib_device_set_netdev(struct ib_device *ib_dev, struct net_device *ndev,
			 unsigned int port)
{
	/* TBD: Need hold ref by calling dev_hold */
			      ...
			      void *arg)
{
	...
	switch (event) {
	case NETDEV_UP:
		dev->port_state = IB_PORT_ACTIVE;
		erdma_port_event(dev, IB_EVENT_PORT_ACTIVE);
		break;
	case NETDEV_DOWN:
		dev->port_state = IB_PORT_DOWN;
...
static int erdma_dev_attrs_init(struct erdma_dev *dev)
{
	...
	if (default_cc >= 0 && default_cc < ERDMA_CC_METHODS_NUM)
		dev->attrs.cc = default_cc;
	else
...
static int erdma_ib_device_add(struct pci_dev *pdev)
{
	...
	ibdev->num_comp_vectors = dev->attrs.max_ceqs - 1;
