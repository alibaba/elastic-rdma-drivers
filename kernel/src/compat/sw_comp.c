...
{
	struct sw_qp *qp = from_timer(qp, t, retrans_timer);
...
void sw_comp_queue_pkt(struct sw_qp *qp, struct sk_buff *skb)
{
	...
	if (must_sched != 0)
		sw_counter_inc(SKB_TO_PKT(skb)->sw, SW_CNT_COMPLETER_SCHED);

	sw_run_task(&qp->comp.task, must_sched);
}

static inline enum comp_state get_wqe(struct sw_qp *qp,
				      struct sw_pkt_info *pkt,
				      struct sw_send_wqe **wqe_p)
{
...
static inline void reset_retry_counters(struct sw_qp *qp)
{
...
static inline enum comp_state check_psn(struct sw_qp *qp,
					struct sw_pkt_info *pkt,
					struct sw_send_wqe *wqe)
{
	s32 diff;
...
static inline enum comp_state check_ack(struct sw_qp *qp,
					struct sw_pkt_info *pkt,
					struct sw_send_wqe *wqe)
{
	...
	case IB_OPCODE_RC_ACKNOWLEDGE:
		...
		case AETH_RNR_NAK:
			sw_counter_inc(sw, SW_CNT_RCV_RNR);
...
static inline enum comp_state do_read(struct sw_qp *qp,
				      struct sw_pkt_info *pkt,
				      struct sw_send_wqe *wqe)
{
	int ret;
	...
	if (wqe->dma.resid == 0 && (pkt->mask & SW_END_MASK))
		return COMPST_COMP_ACK;
	else
		return COMPST_UPDATE_COMP;
}

static inline enum comp_state do_atomic(struct sw_qp *qp,
					struct sw_pkt_info *pkt,
					struct sw_send_wqe *wqe)
{
	int ret;
...
static void make_send_cqe(struct sw_qp *qp, struct sw_send_wqe *wqe,
			  struct sw_cqe *cqe)
{
 ...
 * ---------8<---------8<-------------
 */
static void do_complete(struct sw_qp *qp, struct sw_send_wqe *wqe)
{
	struct sw_dev *sw = to_rdev(qp->ibqp.device);
	struct sw_cqe cqe;
	...
	if (wqe->wr.opcode == IB_WR_SEND ||
		...
		sw_counter_inc(sw, SW_CNT_RDMA_SEND);
...
static inline enum comp_state complete_ack(struct sw_qp *qp,
					   struct sw_pkt_info *pkt,
					   struct sw_send_wqe *wqe)
{
	unsigned long flags;
...
static inline enum comp_state complete_wqe(struct sw_qp *qp,
					   struct sw_pkt_info *pkt,
					   struct sw_send_wqe *wqe)
{
...
static void sw_drain_resp_pkts(struct sw_qp *qp, bool notify)
{
	struct sk_buff *skb;
	struct sw_send_wqe *wqe;
...
int sw_completer(void *arg)
{
	...
	if (!qp->valid || qp->req.state == QP_STATE_ERROR ||
		...
		sw_drain_resp_pkts(qp, qp->valid &&
	...
	while (1) {
		...
		case COMPST_ERROR_RETRY:
			...
			if (qp->comp.retry_cnt > 0) {
				...
				sw_counter_inc(sw, SW_CNT_RETRY_EXCEEDED);
