 ...
 */
struct sw_pkt_info {
	struct sw_dev		*sw;		/* device that owns packet */
	struct sw_qp		*qp;		/* qp that owns packet */
	struct sw_send_wqe	*wqe;		/* send wqe */
...
static inline struct sw_pkt_info *SKB_TO_PKT(struct sk_buff *skb)
{
	BUILD_BUG_ON(sizeof(struct sw_pkt_info) > sizeof(skb->cb));
	return (void *)skb->cb;
}

static inline struct sk_buff *PKT_TO_SKB(struct sw_pkt_info *pkt)
{
 ...
 * sw does not currently support RD transport
...
static inline u8 bth_opcode(struct sw_pkt_info *pkt)
{
	return __bth_opcode(pkt->hdr + pkt->offset);
}

static inline void bth_set_opcode(struct sw_pkt_info *pkt, u8 opcode)
{
	__bth_set_opcode(pkt->hdr + pkt->offset, opcode);
}

static inline u8 bth_se(struct sw_pkt_info *pkt)
{
	return __bth_se(pkt->hdr + pkt->offset);
}

static inline void bth_set_se(struct sw_pkt_info *pkt, int se)
{
	__bth_set_se(pkt->hdr + pkt->offset, se);
}

static inline u8 bth_mig(struct sw_pkt_info *pkt)
{
	return __bth_mig(pkt->hdr + pkt->offset);
}

static inline void bth_set_mig(struct sw_pkt_info *pkt, u8 mig)
{
	__bth_set_mig(pkt->hdr + pkt->offset, mig);
}

static inline u8 bth_pad(struct sw_pkt_info *pkt)
{
	return __bth_pad(pkt->hdr + pkt->offset);
}

static inline void bth_set_pad(struct sw_pkt_info *pkt, u8 pad)
{
	__bth_set_pad(pkt->hdr + pkt->offset, pad);
}

static inline u8 bth_tver(struct sw_pkt_info *pkt)
{
	return __bth_tver(pkt->hdr + pkt->offset);
}

static inline void bth_set_tver(struct sw_pkt_info *pkt, u8 tver)
{
	__bth_set_tver(pkt->hdr + pkt->offset, tver);
}

static inline u16 bth_pkey(struct sw_pkt_info *pkt)
{
	return __bth_pkey(pkt->hdr + pkt->offset);
}

static inline void bth_set_pkey(struct sw_pkt_info *pkt, u16 pkey)
{
	__bth_set_pkey(pkt->hdr + pkt->offset, pkey);
}

static inline u32 bth_qpn(struct sw_pkt_info *pkt)
{
	return __bth_qpn(pkt->hdr + pkt->offset);
}

static inline void bth_set_qpn(struct sw_pkt_info *pkt, u32 qpn)
{
	__bth_set_qpn(pkt->hdr + pkt->offset, qpn);
}

static inline int bth_fecn(struct sw_pkt_info *pkt)
{
	return __bth_fecn(pkt->hdr + pkt->offset);
}

static inline void bth_set_fecn(struct sw_pkt_info *pkt, int fecn)
{
	__bth_set_fecn(pkt->hdr + pkt->offset, fecn);
}

static inline int bth_becn(struct sw_pkt_info *pkt)
{
	return __bth_becn(pkt->hdr + pkt->offset);
}

static inline void bth_set_becn(struct sw_pkt_info *pkt, int becn)
{
	__bth_set_becn(pkt->hdr + pkt->offset, becn);
}

static inline u8 bth_resv6a(struct sw_pkt_info *pkt)
{
	return __bth_resv6a(pkt->hdr + pkt->offset);
}

static inline void bth_set_resv6a(struct sw_pkt_info *pkt)
{
	__bth_set_resv6a(pkt->hdr + pkt->offset);
}

static inline int bth_ack(struct sw_pkt_info *pkt)
{
	return __bth_ack(pkt->hdr + pkt->offset);
}

static inline void bth_set_ack(struct sw_pkt_info *pkt, int ack)
{
	__bth_set_ack(pkt->hdr + pkt->offset, ack);
}

static inline void bth_set_resv7(struct sw_pkt_info *pkt)
{
	__bth_set_resv7(pkt->hdr + pkt->offset);
}

static inline u32 bth_psn(struct sw_pkt_info *pkt)
{
	return __bth_psn(pkt->hdr + pkt->offset);
}

static inline void bth_set_psn(struct sw_pkt_info *pkt, u32 psn)
{
	__bth_set_psn(pkt->hdr + pkt->offset, psn);
}

static inline void bth_init(struct sw_pkt_info *pkt, u8 opcode, int se,
			    int mig, int pad, u16 pkey, u32 qpn, int ack_req,
			    u32 psn)
{
	struct sw_bth *bth = (struct sw_bth *)(pkt->hdr + pkt->offset);
...
static inline u8 rdeth_een(struct sw_pkt_info *pkt)
{
	return __rdeth_een(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_RDETH]);
}

static inline void rdeth_set_een(struct sw_pkt_info *pkt, u32 een)
{
	__rdeth_set_een(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_RDETH], een);
}
...
static inline u32 deth_qkey(struct sw_pkt_info *pkt)
{
	return __deth_qkey(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_DETH]);
}

static inline void deth_set_qkey(struct sw_pkt_info *pkt, u32 qkey)
{
	__deth_set_qkey(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_DETH], qkey);
}

static inline u32 deth_sqp(struct sw_pkt_info *pkt)
{
	return __deth_sqp(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_DETH]);
}

static inline void deth_set_sqp(struct sw_pkt_info *pkt, u32 sqp)
{
	__deth_set_sqp(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_DETH], sqp);
}
...
static inline u64 reth_va(struct sw_pkt_info *pkt)
{
	return __reth_va(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_RETH]);
}

static inline void reth_set_va(struct sw_pkt_info *pkt, u64 va)
{
	__reth_set_va(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_RETH], va);
}

static inline u32 reth_rkey(struct sw_pkt_info *pkt)
{
	return __reth_rkey(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_RETH]);
}

static inline void reth_set_rkey(struct sw_pkt_info *pkt, u32 rkey)
{
	__reth_set_rkey(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_RETH], rkey);
}

static inline u32 reth_len(struct sw_pkt_info *pkt)
{
	return __reth_len(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_RETH]);
}

static inline void reth_set_len(struct sw_pkt_info *pkt, u32 len)
{
	__reth_set_len(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_RETH], len);
}
...
static inline u64 atmeth_va(struct sw_pkt_info *pkt)
{
	return __atmeth_va(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_ATMETH]);
}

static inline void atmeth_set_va(struct sw_pkt_info *pkt, u64 va)
{
	__atmeth_set_va(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_ATMETH], va);
}

static inline u32 atmeth_rkey(struct sw_pkt_info *pkt)
{
	return __atmeth_rkey(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_ATMETH]);
}

static inline void atmeth_set_rkey(struct sw_pkt_info *pkt, u32 rkey)
{
	__atmeth_set_rkey(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_ATMETH], rkey);
}

static inline u64 atmeth_swap_add(struct sw_pkt_info *pkt)
{
	return __atmeth_swap_add(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_ATMETH]);
}

static inline void atmeth_set_swap_add(struct sw_pkt_info *pkt, u64 swap_add)
{
	__atmeth_set_swap_add(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_ATMETH], swap_add);
}

static inline u64 atmeth_comp(struct sw_pkt_info *pkt)
{
	return __atmeth_comp(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_ATMETH]);
}

static inline void atmeth_set_comp(struct sw_pkt_info *pkt, u64 comp)
{
	__atmeth_set_comp(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_ATMETH], comp);
}
...
static inline u8 aeth_syn(struct sw_pkt_info *pkt)
{
	return __aeth_syn(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_AETH]);
}

static inline void aeth_set_syn(struct sw_pkt_info *pkt, u8 syn)
{
	__aeth_set_syn(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_AETH], syn);
}

static inline u32 aeth_msn(struct sw_pkt_info *pkt)
{
	return __aeth_msn(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_AETH]);
}

static inline void aeth_set_msn(struct sw_pkt_info *pkt, u32 msn)
{
	__aeth_set_msn(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_AETH], msn);
}
...
static inline u64 atmack_orig(struct sw_pkt_info *pkt)
{
	return __atmack_orig(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_ATMACK]);
}

static inline void atmack_set_orig(struct sw_pkt_info *pkt, u64 orig)
{
	__atmack_set_orig(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_ATMACK], orig);
}
...
static inline __be32 immdt_imm(struct sw_pkt_info *pkt)
{
	return __immdt_imm(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_IMMDT]);
}

static inline void immdt_set_imm(struct sw_pkt_info *pkt, __be32 imm)
{
	__immdt_set_imm(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_IMMDT], imm);
}
...
static inline u32 ieth_rkey(struct sw_pkt_info *pkt)
{
	return __ieth_rkey(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_IETH]);
}

static inline void ieth_set_rkey(struct sw_pkt_info *pkt, u32 rkey)
{
	__ieth_set_rkey(pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_IETH], rkey);
}

enum sw_hdr_length {
	SW_BTH_BYTES		= sizeof(struct sw_bth),
	SW_DETH_BYTES		= sizeof(struct sw_deth),
	SW_IMMDT_BYTES		= sizeof(struct sw_immdt),
	SW_RETH_BYTES		= sizeof(struct sw_reth),
	SW_AETH_BYTES		= sizeof(struct sw_aeth),
	SW_ATMACK_BYTES	= sizeof(struct sw_atmack),
	SW_ATMETH_BYTES	= sizeof(struct sw_atmeth),
	SW_IETH_BYTES		= sizeof(struct sw_ieth),
	SW_RDETH_BYTES		= sizeof(struct sw_rdeth),
};

static inline size_t header_size(struct sw_pkt_info *pkt)
{
	return pkt->offset + sw_opcode[pkt->opcode].length;
}

static inline void *payload_addr(struct sw_pkt_info *pkt)
{
	return pkt->hdr + pkt->offset
		+ sw_opcode[pkt->opcode].offset[SW_PAYLOAD];
}

static inline size_t payload_size(struct sw_pkt_info *pkt)
{
	return pkt->paylen - sw_opcode[pkt->opcode].offset[SW_PAYLOAD]
		- bth_pad(pkt) - SW_ICRC_SIZE;
}
